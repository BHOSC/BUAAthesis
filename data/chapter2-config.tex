...
需要经过以下步骤（以sample-bachelor.tex为例）：
\begin{lstlisting}[language=bash,caption={编译步骤}]
    latex sample-bachelor.tex
...
进入cmd（Ctrl+R，然后输入cmd），cd到BUAAthesis对应目录，
如{\verb D:\BUAAthesis\ }，然后运行{\verb msmake }即可。

\subsection{Makefile [ Windows(cygwin) / Linux / MacOS ]}
需要要你的命令行环境支持Make，cd到BUAAthesis相应目录，目前支持以下功能:
\begin{lstlisting}[language=bash,caption={make 命令}]
